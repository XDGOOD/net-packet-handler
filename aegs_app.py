...
"""
==============================================================================
AEGS Desktop Client (Windows & Cross-Platform) -- Titan Edition v6
==============================================================================
Features:
...
- Network Virtualization:
  - High-performance Wintun support + zero-admin system proxy fallback
"""
...
class AegsApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("AEGS VPN — Titan v6.0")
    ...
    def _setup_styles(self):
        ...
        style.configure("TNotebook.Tab", background="#1C212E", foreground="#8E99B0", padding=[15, 8], font=("Segoe UI", 10, "bold"))
    ...
    def _build_ui(self):
        ...
        self.lbl_proto = ttk.Label(header, text="v6.0 • RFC 9000 QUIC Stealth", background="#12141A", foreground="#00D26A", font=("Segoe UI", 9, "bold"))
    ...
    def _build_connect_tab(self):
        ...
        self.lbl_active_server.pack(anchor="w", pady=(2, 15))
        ...
        self.btn_connect.pack(fill="x", pady=15)
        ...
        det_frame.pack(fill="x", pady=(15, 0))

        ttk.Label(det_frame, text="Режим защиты: Обход ТСПУ / RFC 9000 QUIC Camouflage", style="Dim.TLabel").pack(anchor="w")
        ttk.Label(det_frame, text="Шифрование: ChaCha20-Poly1305 + Stateless Cookies (Anti-DDoS)", style="Dim.TLabel").pack(anchor="w")
        ttk.Label(det_frame, text="Адаптер: Wintun High-Speed Virtual Adapter (300-900+ Mbps)", style="Dim.TLabel").pack(anchor="w")

        self.refresh_active_display()

    def _build_servers_tab(self):
        frame = ttk.Frame(self.tab_servers, style="Dark.TFrame", padding=10)
        frame.pack(fill="both", expand=True)

        ttk.Label(frame, text="Выберите или добавьте свой VPS сервер:", background="#12141A", foreground="#FFFFFF", font=("Segoe UI", 11, "bold")).pack(anchor="w", pady=(0, 10))
        ...
        btn_select = tk.Button(btn_box, text="Выбрать активным", command=self.select_server, bg="#1C212E", fg="#FFFFFF", relief="flat", font=("Segoe UI", 9, "bold"), padx=10, pady=5)
        btn_select.pack(side="left", padx=(0, 5))

        btn_add = tk.Button(btn_box, text="+ Добавить свой VPS", command=self.add_custom_server_dialog, bg="#0D6EFD", fg="#FFFFFF", relief="flat", font=("Segoe UI", 9, "bold"), padx=10, pady=5)
        btn_add.pack(side="left", padx=5)

        btn_del = tk.Button(btn_box, text="Удалить", command=self.delete_server, bg="#DC3545", fg="#FFFFFF", relief="flat", font=("Segoe UI", 9, "bold"), padx=10, pady=5)
    ...
    def _build_mobile_tab(self):
        ...
        ttk.Label(card, text="Используйте приложение AEGS или Amnezia на телефоне", style="Card.TLabel", font=("Segoe UI", 11, "bold")).pack(anchor="w", pady=(2, 10))

        ttk.Label(card, text="Скопируйте ключ подключения для телефона:", style="Dim.TLabel").pack(anchor="w", pady=(10, 2))

        self.txt_uri = tk.Text(card, height=4, bg="#12141A", fg="#00D26A", relief="flat", font=("Consolas", 9), wrap="break")
    ...
    def _on_connected(self):
        self.lbl_status.config(text="● Защищено (AEGS v6 Stealth)", foreground="#00D26A")
        self.btn_connect.config(text="ОТКЛЮЧИТЬСЯ", bg="#DC3545", activebackground="#BB2D3B", state="normal")
        self.lbl_ping.config(text="Пинг: 28 мс")
